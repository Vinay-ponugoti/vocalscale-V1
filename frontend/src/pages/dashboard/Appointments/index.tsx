...
import {
  ...
  Calendar as CalendarIcon, Trash2, AlertTriangle, CheckCircle2
} from 'lucide-react';
...
const FullScreenAppointments: React.FC = () => {
  ...
  // Initialize checks from localStorage on mount
  useEffect(() => {
    const savedPreference = localStorage.getItem('skipDeleteConfirm');
  ...
  const handleDelete = async () => {
    if (!selectedAppointment || !deleteAppointment) return;
                      ...
                      {isEditing ? (
                        ...
                        <>
                          <button
                            ...
                            className="px-4 py-2.5 font-bold border-2 rounded-xl active:scale-95 transition-all text-sm flex items-center justify-center gap-2 group/delete"
                            style={{ color: DS.danger, backgroundColor: DS.dangerBg, borderColor: DS.dangerBg }}
                            onMouseEnter={(e) => {
                              e.currentTarget.style.borderColor = DS.danger;
                            }}
                            onMouseLeave={(e) => {
                              e.currentTarget.style.borderColor = DS.dangerBg;
                            }}
          ...
          <div className="relative z-[150]" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            ...
            <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
              <div className="flex min-h-full items-center justify-center p-4 text-center">
                <div
                  className="relative transform overflow-hidden rounded-2xl text-left shadow-2xl transition-all sm:w-full sm:max-w-md animate-in zoom-in-95 duration-200"
                  style={{ backgroundColor: DS.white, borderColor: DS.border }}
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="p-6">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#FEF2F2' }}>
                        <AlertTriangle className="w-6 h-6 text-red-600" />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-slate-900 leading-tight">Delete Appointment?</h3>
                    ...
                    <div className="bg-red-50 rounded-xl p-4 mb-5 border border-red-100">
                      <p className="text-sm text-red-800 font-medium">
                        Warning: This appointment will be permanently deleted and cannot be retrieved. Any synced Google Calendar events will also be removed.
                      </p>
                    </div>

                    <div className="flex items-center gap-2 mb-6 cursor-pointer" onClick={() => setDontAskAgain(!dontAskAgain)}>
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${dontAskAgain ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-300'}`}>
                        {dontAskAgain && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                      </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        type="button"
                        disabled={isDeleting}
                        className="flex-1 px-4 py-2.5 rounded-xl font-bold text-sm text-white bg-red-600 hover:bg-red-700 shadow-md shadow-red-200 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
