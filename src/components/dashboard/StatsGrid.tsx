...
const StatCard: React.FC<StatCardProps> = ({ 
  ...
  return (
    <Card className={`relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 border-slate-100 shadow-sm ${
      highlight ? 'ring-2 ring-orange-500/20' : ''
    }`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm font-bold text-slate-500 tracking-tight uppercase">{label}</CardTitle>
        <div className={`p-2 rounded-xl ${
          highlight ? 'bg-orange-50 text-orange-600' : 'bg-slate-50 text-slate-600'
        }`}>
      ...
      <CardContent>
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center justify-between mt-2">
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{description || 'Total recorded'}</p>
            {badge ? (
