...
export const LivePreview = () => {
  ...
  return (
    <div className="w-full flex flex-col h-full">
      ...
      <div className="relative bg-slate-900 rounded-xl shadow-sm border border-slate-800 flex flex-col flex-1 overflow-hidden">
        ...
        {/* AI Identity - Compact */}
        <div className="flex items-center gap-3 p-3 border-b border-slate-800 bg-slate-900/50 backdrop-blur-sm shrink-0">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-sm shrink-0">
            <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-white text-xs font-bold truncate">{businessName}</h3>
            <div className="flex items-center gap-1">
              <span className="text-[10px] text-indigo-400 font-medium">AI Assistant</span>
        ...
        <div className="flex-1 p-3 space-y-2 overflow-y-auto bg-slate-950/30 scrollbar-hide">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              <div className={`max-w-[90%] text-[11px] leading-snug px-3 py-2 ${
                ...
                : 'bg-slate-800 text-slate-100 border border-slate-700 rounded-lg rounded-tl-sm shadow-sm'
              }`}>
                {msg.text}
              </div>
            </div>
